...
class RepoTareas extends CollectionBasedRepo<Tarea> {
	...
	private new() {
		...
		this.crearTarea("Desarrollar componente de envio de mails", "Marcos Pavelek", new Date(), "Iteracion 1", 0)
		this.crearTarea("Implementar single sign on desde la extranet", null, new Date(9, 9, 114), "Iteracion 1", 76)
		this.crearTarea("Cancelar pedidos que esten pendientes desde hace 2 meses", "Rodrigo Grisolia", new Date(3, 2, 115),
			"Iteracion 1", 22)
		this.crearTarea("Mostrar info del pedido cuando esta finalizado", null, new Date(8, 10, 114), "Iteracion 2", 90)
	}
